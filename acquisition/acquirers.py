...
def select(ensemble, X_train, X_pool, batch_acquire, selection_method='random', **cfg):
    ...
    if selection_method == "random":
        ...
        with torch.no_grad():
            ...
            features_all = features_all.flatten(start_dim=0, end_dim=1) # [bs_all*N, dim]
            indices_all = indices_all.flatten(start_dim=0, end_dim=1) # [bs_all*N]
        
        for _ in range(batch_acquire):
            rem_indices = torch.nonzero(torch.isin(indices_all, rem)) # [bs_rem]
            sel_indices = torch.nonzero(torch.isin(indices_all, sel)) # [bs_sel]
            if sel.shape[0] == 0:
                new = torch.argmax(torch.norm(features_all[rem_indices], dim=1))
                new = rem_indices[new]
                new = indices_all[new]
            else:
                distances = torch.cdist(features_all[sel_indices].unsqueeze(0), features_all[rem_indices].unsqueeze(0)).squeeze(0).square() # [bs_sel, bs_rem]
                centers = torch.argmin(distances, dim=0) # [bs_rem]: indices in bs_sel
...
def select_time(ensemble, Y, train_nts, batch_acquire, selection_method='random', **cfg):
    device = cfg.get('device', 'cpu')
        ...
        for b in range(bs):
            ...
            for model in ensemble:
                ...
                for t in range(nt-train_nts[b]):
                    trajectory.append(model(trajectory[-1].unsqueeze(0)).squeeze(0))
                trajectory.pop(0)
